...
const Page = ({ params }) => {
    ...
    try {
      ...
      if (response.ok) {
        setHelperDetails({
          ...
          helperNID: data?.helperNID,
        });
      } else {
        console.log(data.message || "Failed to fetch drivers!");
        router.push(`/dashboard/${adminId}/helper-info`);
      }
    } catch (error) {
  ...
  const updateHelperHandler = async (e) => {
    ...
    try {
      ...
      console.log(data);
      if (response.ok) {
        toast.success("Heper Updated successfully!");
      } else {
        toast.error(data.message || "Failed to update helper!");
      }
    } catch (error) {
      toast.error("Something went wrong! Please try again.");
      console.error("Error submitting update helper:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      {isLoading1 ? (
        ...
        <form onSubmit={updateHelperHandler} className="space-y-3">
          ...
          <div className=" pt-5 ">
            ...
            >
              {loading ? <ButtonLoading /> : "Update Helper Info"}
            </motion.button>
          </div>
        </form>
      )}
